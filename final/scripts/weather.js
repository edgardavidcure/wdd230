...
async function getForecastDays() {
  try {
    ...
    const filteredForecasts = data.list.filter(forecast => {
      ...
      const now = new Date();
      const diffInDays = (forecastDate.getTime() - now.getTime()) / (1000 * 3600 * 24);
    ...
    Object.keys(groupedByDate).forEach(date => {
      ...
      const minTemperatures = forecasts.map(forecast => forecast.main.temp_min);
      const maxTemperatures = forecasts.map(forecast => forecast.main.temp_max);
      const avgMinTemperature = minTemperatures.length ? Math.round(minTemperatures.reduce((a, b) => a + b) / minTemperatures.length) : null;
      const avgMaxTemperature = maxTemperatures.length ? Math.round(maxTemperatures.reduce((a, b) => a + b) / maxTemperatures.length) : null;
      avgTemperatures[date] = { minTemperature: Math.min(...minTemperatures), maxTemperature: Math.max(...maxTemperatures), avgMinTemperature, avgMaxTemperature };
    });
...
async function getCurrentTemp(){
    const response = await fetch("https://api.openweathermap.org/data/2.5/weather?q=Carlsbad&appid=582b55efa5e27910234adbbf3225cdad");
    const data = await response.json();
...
function drawWeatherForecast(days, forecast){
    ...
    day1MinTemp.innerHTML = `Min: ${kelvinToFahrenheit(forecast[Object.keys(forecast)[1]].minTemperature)}&degF`
    day1MaxTemp.innerHTML = `Max: ${kelvinToFahrenheit(forecast[Object.keys(forecast)[1]].maxTemperature)}&degF`
    day2MinTemp.innerHTML = `Min: ${kelvinToFahrenheit(forecast[Object.keys(forecast)[2]].minTemperature)}&degF`
    day2MaxTemp.innerHTML = `Max: ${kelvinToFahrenheit(forecast[Object.keys(forecast)[2]].maxTemperature)}&degF`
    day3MinTemp.innerHTML = `Min: ${kelvinToFahrenheit(forecast[Object.keys(forecast)[3]].minTemperature)}&degF`
    day3MaxTemp.innerHTML = `Max: ${kelvinToFahrenheit(forecast[Object.keys(forecast)[3]].maxTemperature)}&degF`
